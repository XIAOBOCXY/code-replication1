...
class ExpressionLossNet(nn.Module):
    ...
    def __init__(self):
        ...
        path_ckpt = "/data/Workspace/emoca/gdl_apps/EmotionRecognition/checkpoints/ResNet50/checkpoints/deca-epoch=01-val_loss_total/dataloader_idx_0=1.27607644.ckpt"
        self.backbone = load_ResNet50Model().eval() #out: 2048
        ckpt = torch.load(path_ckpt)['state_dict']
        ckpt ={key.replace('backbone.', ''):ckpt[key] for key in ckpt}
...
if __name__ == '__main__':
    inputs_d = torch.zeros((2,3,224,224))
    # backbone = load_ResNet50Model()
    backbone = ExpressionLossNet()
    torch.load("/data/Workspace/emoca/gdl_apps/EmotionRecognition/checkpoints/ResNet50/checkpoints/deca-epoch=01-val_loss_total/dataloader_idx_0=1.27607644.ckpt")['state_dict']